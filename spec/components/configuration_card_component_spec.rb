...
RSpec.describe ConfigurationCardComponent, type: :component do
  ...
  it 'renders default configuration card component' do
    render_inline(
      described_class.new(
        ...
        options: required_params[:options]
      )
    )
    ...
    expect(page).to have_link href: required_params[:options][:href1]
  end
  ...
  it 'renders configuration card component with full params' do
    ...
    expect(page).to have_link href: options[:options][:href1]
    expect(page).to have_content 'Crear cliente'
    expect(page).to have_link href: options[:options][:href2]
  end
end
