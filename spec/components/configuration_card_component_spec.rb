...
RSpec.describe ConfigurationCardComponent, type: :component do
  ...
  it 'renders default configuration card component' do
    ...
    expect(page).to have_link href: required_params[:options][:href_2]
  end

  it 'renders configuration card component with full params' do
    options = required_params[:options].merge(full_params)
    render_inline(described_class.new(title: required_params[:title], description: required_params[:description], options: options))
